...
let component_with_index' t ~wrap_remove (local_ graph) =
    ...
    Bonsai.assoc
      ...
      ~f:
        (fun
          ...
          (local_ graph)
        ->
        let result, reset = Bonsai.with_model_resetter ~f:(t ~key) graph in
        let%arr out = result
        and reset
        and key
...
let component t =
  component_with_index' (fun ~key:_ graph -> t graph) ~wrap_remove:(fun a _ -> a)
;;

let component_with_key t = component_with_index' t ~wrap_remove:(fun a _ -> a)
let component' t = component_with_index' (fun ~key:_ graph -> t graph)
