...
and render
  : ?container_attr:Vdom.Attr.t -> ?top_level:bool -> closed Builder.t -> Vdom.Node.t
  =
  fun ?(container_attr = Vdom.Attr.empty) ?(top_level = true) (Terminal layout) ->
  ...
  let attr =
    ...
    Vdom.Attr.many
      ...
      ; (if top_level then Style.root else Vdom.Attr.empty)
      ]
  in
