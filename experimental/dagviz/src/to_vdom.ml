...
module Make (Name : Types.Name) = struct
  ...
  let to_vdom
    ...
    ~(node_to_vdom : Name.t Bonsai.t -> local_ Bonsai.graph -> Vdom.Node.t Bonsai.t)
    ~(edge_to_svg :
    ...
    : local_ Bonsai.graph -> (Vdom.Node.t * Name.Count.t) Bonsai.t
    =
    fun (local_ graph) ->
  ...
  let to_bindgen (type a) (input : a t Bonsai.t) (local_ _graph) =
  ...
  let create
    ...
    | Ok bindgen ->
      let out (local_ graph) =
        let%sub vdom, curr_id =
          to_vdom
            ~direction
            ~curr_id
            ~node_to_vdom:(fun key (local_ graph) ->
              let data =
