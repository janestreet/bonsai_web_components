...
module type S = sig
  ...
  val all : Bonsai.graph -> (int, Row.t, column_id) Table.Columns.t
  val with_column_groups : Bonsai.graph -> (int, Row.t, column_id) Table.Columns.t
end
...
let new_api ~counters_in_cells ~cols ~render_cell_kind ~duplicate_col : (module S) =
  (module struct
    ...
    let stateful_rows _k row graph =
      let build_button =
        match counters_in_cells with
        | true ->
          let state, incr_cell =
    ...
    let stateful_cells col _k row graph =
      let counter =
        match counters_in_cells with
        | true -> counter graph
        | false -> return (Vdom.Node.none_deprecated [@alert "-deprecated"])
...
module For_component = struct
  type t =
    | T :
        { columns : Bonsai.graph -> (int, Row.t, 'column_id) Table.Columns.t
