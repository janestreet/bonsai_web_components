...
(** For each leaf, [column_names] returns a list like
    [group_header; group_header; ...; leaf_header], where the group labels are that leaf's
    ancestors, ordered left to right from most to least distant. Used for rendering column
    groups in tests. *)
