...
let component
  ...
  match kind with
  | None ->
    fun _ _ ~collated:_ ~leaves:_ ~range:_ ~scroll_to:_ _graph ->
      Bonsai.return { focus = (); visually_focused = Nothing_focused }
  | By_row { on_change; compute_presence; key_rank } ->
    fun key _ ~collated ~leaves:_ ~range ~scroll_to graph ->
  ...
  | By_cell { on_change; compute_presence; key_rank } ->
    fun key column ~collated ~leaves ~range ~scroll_to graph ->
      let columns =
        let%arr leaves in
