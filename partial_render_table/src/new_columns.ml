...
module Shared = struct
  ...
  let create
    ...
    =
    fun column_id { Column_structure.cols; initial_widths; resizable } ~render_cell ->
    ...
    { column_id; columns; render_cell; initial_widths; resizable }
  ;;
end
...
module Expert = struct
  module T = struct
    type ('key, 'data, 'column_id, 'column_id_cmp) t =
      { shared : ('key, 'data, 'column_id, 'column_id_cmp) Shared.t
    ...
    let headers
      ...
      =
      fun { render_header; shared = { columns; column_id; initial_widths; resizable; _ } }
        graph ->
      match columns with
      | Static (cols, _) ->
      ...
      | Dynamic (cols, _, col_ids_map) ->
        ...
        in
        let render_header col_id (_ : unit Bonsai.t) graph = render_header col_id graph in
        let%arr cols
      ...
      | Dynamic (_, all_column_ids, col_ids_map) ->
        ...
        (match render_cell with
         | Stateful_cells render_cell ->
           assoc_on_rows ~f:(fun key data graph ->
             Bonsai.assoc
               (module Col_id)
               col_ids_map
               ~f:(fun col_id _ graph -> render_cell col_id key data graph)
               graph)
    ...
    let instantiate_cells { shared; _ } key_cmp rows =
      instantiate_cells' shared key_cmp rows
    ;;
  end
  ...
  let build
    ...
    fun column_id ~columns ~render_header ~render_cell ->
      let shared = Shared.create column_id columns ~render_cell in
      let value = { T.render_header; shared } in
...
module Basic = struct
  module T = struct
    ...
    let instantiate_cells { shared; _ } key_cmp rows =
      Expert.T.instantiate_cells' shared key_cmp rows
    ;;
      ...
      : type key data column_id column_id_cmp.
        ...
        -> ((column_id, (key, data) Sort_kind.t, column_id_cmp) Map.t
           * column_id Header_tree.t)
             Bonsai.t
      =
      fun ~multisort_columns_when { render_header; sorts; shared } sortable_header graph ->
      let module Col_id = (val shared.column_id) in
      let sorts =
        (match sorts, shared.columns with
         ...
         | Some sorts, Dynamic (_, _, columns_map) ->
           Bonsai.assoc
             ...
             graph)
        |> fun x -> Bonsai.Map.filter_map x ~f:Fn.id graph
      in
      let headers =
        let render_header col graph =
          let%arr original = render_header col graph
          and col
          and sorts
          and multisort_columns_when
          ...
          Sortable.Header.Expert.default_click_handler
            ...
            original
        in
        Expert.T.headers { render_header; shared } graph
      in
      Bonsai.both sorts headers
    ;;
  end
  ...
  let build
    ...
    fun ?sorts column_id ~columns ~render_header ~render_cell ->
      let shared = Shared.create column_id columns ~render_cell in
      let value = { T.render_header; sorts; shared } in
