...
module Dynamic_cells = struct
  module T = struct
    ...
    let headers t (local_ _graph) = headers t
...
module Dynamic_experimental = struct
  module T = struct
    ...
    let headers
      ...
      =
      fun { column_id; columns; render_header; render_cell = _ } (local_ graph) ->
...
module Dynamic_columns = struct
  module T = struct
    ...
    let headers t (local_ _graph) = Bonsai.map ~f:translate t
  ...
  let column
    ?(initial_width = New_columns.Column_structure.default_initial_width)
    ?(visible = true)
    ?(resizable = true)
    ~header
    ~cell
...
module Dynamic_cells_with_sorter = struct
  ...
  module W = struct
    let headers_and_sorters ~multisort_columns_when t sortable_state (local_ graph) =
      let sorters, tree =
  ...
  module T = struct
    ...
    let instantiate_cells
      ...
      Bonsai.Expert.assoc_on
        ...
        rows
        ~get_model_key:(fun _ (k, _) -> k)
        ~f:(fun _key_key data (local_ graph) ->
          let%sub key, data = data in
          let rendered =
            Bonsai.assoc_list
              (module Col_id)
