  ...
  module Dir = struct
    module V1 = struct
      ...
      let%expect_test _ =
        ...
        [%expect {| db71eae4481c5e9e89203381f641dac5 |}]
      ;;
    end
  end
  ...
  module Order = struct
    module V1 = struct
      ...
      let%expect_test _ =
        ...
        [%expect {| aedd1b5849149879d09c261d4e0c9e68 |}]
      ;;
    end
  end
end
...
module Order = struct
  ...
  let apply_action t ~equal (action : 'col_id Action.t) =
    let cycle_sort_direction id (cycle_behavior : Cycle_behavior.t) =
      match cycle_behavior, List.Assoc.find ~equal t id with
      | Asc_to_desc_to_none, None -> [ id, `Asc ]
      | Asc_to_desc_to_none, Some `Asc -> [ id, `Desc ]
      | Asc_to_desc_to_none, Some `Desc -> []
      | Desc_to_asc_to_none, None -> [ id, `Desc ]
      | Desc_to_asc_to_none, Some `Asc -> []
      | Desc_to_asc_to_none, Some `Desc -> [ id, `Asc ]
    in
    match action with
    | Set_sort (id, cycle_behavior) -> cycle_sort_direction id cycle_behavior
    ...
    | Add_sort (id, cycle_behavior) ->
      ...
      then (
        ...
        keep @ cycle_sort_direction id cycle_behavior)
      else t @ cycle_sort_direction id cycle_behavior
  ;;
