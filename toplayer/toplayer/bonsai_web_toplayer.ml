...
module Restore_focus_on_close = Bonsai_web_toplayer_private_vdom.Restore_focus_on_close
...
let focus_on_open_attr = function
  ...
  | true -> Bonsai_web_toplayer_private_vdom.For_bonsai_web_toplayer.focus_popover_on_open
;;
...
module Close_on_click_outside = struct
  ...
  let is_target_inside_a_popover ~target =
    ...
    |> Option.bind
         ~f:
           Bonsai_web_toplayer_private_vdom.For_bonsai_web_toplayer
...
module Popover = struct
  let always_open
    ...
    Bonsai_web_toplayer_private_vdom.popover
  ...
  let always_open_unpositioned
    ...
    Bonsai_web_portal.component
      (fun graph ->
        ...
        and arrow
        and content = content graph in
        Bonsai_web_toplayer_private_vdom.For_bonsai_web_toplayer.popover_custom
          ~popover_attrs:(focus_on_open_attr focus_on_open :: autoclose :: attrs)
          ?overflow_auto_wrapper
          ?arrow
  ...
  let always_open_css ~attrs ?autoclose ?focus_on_open ~content graph =
    let attrs =
      let%arr attrs in
      (* Other popovers do this in the auto-positioning hook's [prepare]. *)
      Bonsai_web_toplayer_private_vdom.For_bonsai_web_toplayer.show_on_mount :: attrs
    in
  ...
  let always_open_virtual
    ...
    let positioning_attr =
      ...
      Bonsai_web_toplayer_private_floating.position_me
        ~prepare:Bonsai_web_toplayer_private_vdom.For_bonsai_web_toplayer.show_popover
        ~arrow_selector:
          Bonsai_web_toplayer_private_vdom.For_bonsai_web_toplayer.arrow_selector
        ?position
        ?alignment
        ?offset
        ?match_anchor_side_length
  ...
  let always_open
    ...
    Bonsai_web_portal.component
      (fun graph ->
        ...
        and autoclose
        and content = content graph in
        Bonsai_web_toplayer_private_vdom.For_bonsai_web_toplayer.modal
