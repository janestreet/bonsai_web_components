...
let assert_open_but_not_shown ~(here : [%call_pos]) ?(selector = default_selector) handle =
...
let assert_open_and_shown ~(here : [%call_pos]) ?(selector = default_selector) handle =
  Handle.query_selector
    handle
...
let assert_not_in_dom ~(here : [%call_pos]) ?(selector = default_selector) handle =
