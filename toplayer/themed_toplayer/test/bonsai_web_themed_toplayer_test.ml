...
module Helper = struct
  type positioning =
    { position : Bonsai_web_toplayer_private_vdom.Position.t
    ; alignment : Bonsai_web_toplayer_private_vdom.Alignment.t
    ; offset : Bonsai_web_toplayer_private_vdom.Offset.t
    ; match_anchor_side_length :
        Bonsai_web_toplayer_private_vdom.Match_anchor_side.t option
    }
  ...
  module Anchored_popover = struct
    ...
    let best_effort_wrap_content_in_popover_vdom
      ...
      | Some { position; alignment; offset; match_anchor_side_length } ->
        ...
        in
        Bonsai_web_toplayer_private_vdom.For_testing_bonsai_web_toplayer
        .wrap_anchored_popover
          ~restore_focus_on_close:
            (Bonsai_web_toplayer_private_vdom.Restore_focus_on_close.Yes
  ...
  module Modal = struct
    ...
    let is_modal =
      has_attr
        ~attr_name:
          Bonsai_web_toplayer_private_vdom.For_testing_bonsai_web_toplayer.modal_attr_name
    ;;
  ...
  let rec extract_anchored_single_node node_helper =
    let hook_inputs =
      Node_helpers.get_hook_value_opt
        node_helper
        ~name:Bonsai_web_toplayer_private_vdom.For_testing_popover_hook.hook_name
        ~type_id:Bonsai_web_toplayer_private_vdom.For_testing_popover_hook.type_id
    in
    ...
    let positioning =
      let%map.Option hook_inputs =
        Node_helpers.get_hook_value_opt
          root_helper
          ~name:
            Bonsai_web_toplayer_private_floating.For_testing_position_me_hook.hook_name
          ~type_id:
            Bonsai_web_toplayer_private_floating.For_testing_position_me_hook.type_id
      in
  ...
  let get_toplayer_elements app_vdom =
    ...
    let virtual_and_modals =
      let%arr portals =
        Bonsai.Expert.Var.value Bonsai_web_portal.For_testing.active_portals
      in
      Map.data portals
      |> List.map ~f:Bonsai_web_portal_private.For_testing.vdom
...
open Bonsai_web_themed_toplayer
