open! Core
open! Bonsai_web
module Styling = Bonsai_web_themed_toplayer_styling

(** NOTE: This library is implemented in terms of [Bonsai_web_toplayer]. The
    implementation in [Bonsai_web_themed_toplayer.ml] is mainly a wrapper, providing
    [Config.t]s, themability, etc.

    Toplayer elements will not show up in tests by default, because they are portalled
    outside of the app root. You should use [bonsai_web_themed_toplayer_test] as a helper
    library in your testing.

    [bonsai_web_themed_toplayer] contains pure vdom tooltips, and stateful Bonsai popovers
    and modals: UI elements that appear in the browser top layer, on top of everything
    else in your web UI.
    ...
    Authors of reusable components might also be interested in the [bonsai_web_toplayer]
    library, which is the foundation for this one. *)

module Position = Bonsai_web_toplayer.Position
module Alignment = Bonsai_web_toplayer.Alignment
module Offset = Bonsai_web_toplayer.Offset
module Restore_focus_on_close = Bonsai_web_toplayer.Restore_focus_on_close
...
module Match_anchor_side = Bonsai_web_toplayer.Match_anchor_side

module Anchor : sig
  type t = Bonsai_web_toplayer.Anchor.t [@@deriving sexp_of]
...
module Close_on_click_outside : sig
  ...
  type t = Bonsai_web_toplayer.Close_on_click_outside.t =
    | Yes
...
module Controls : sig
  type t = Bonsai_web_toplayer.Controls.t =
