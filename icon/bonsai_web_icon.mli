...
(** A [Bonsai_web_icon] is an abstract wrapper around an svg icon. It's designed so that
...
(** Render a given [Bonsai_web_icon.t] as a [Vdom.Node.t].
...
(** Functions in [Expert] can produce unsafe code due to the conversion of strings to
    [Vdom]. Use with caution, and reach out to Bonsai devs if unsure. *)
module Expert : sig
  (** Create a [Bonsai_web_icon]. *)
  val create
    :  name:string
    -> svg_path:string
