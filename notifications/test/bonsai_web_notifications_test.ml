...
let%expect_test "render some notifications and test that they close as expected" =
  ...
  let handle =
    Handle.create (module Notification_spec) (fun (local_ graph) ->
      let notifications =
...
module%test Compare_send_notifications_and_send_notifications_deprecated = struct
  ...
  let component ~send_notification (local_ graph) =
    let notifications = Bonsai_web_notifications.component ~equal:[%equal: unit] graph in
  ...
  let old_send_notification ~notifications =
    ...
    let%bind.Effect _id = Bonsai_web_notifications.send_notification notifications () in
    Effect.return ()
  ;;

  let new_send_notification ~notifications =
    let%arr send_notification =
      Bonsai_web_notifications.send_notification' notifications
    in
