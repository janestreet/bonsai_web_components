...
val create
  :  key:string Bonsai.Value.t
       (** [key] is a virtualdom concept that allows it to identify which items in a list
  ...
  -> ?custom_legend:
       ...
       (** [custom_legend] defaults to Default_legend. If you don't want that legend,
           you're free to pass in your own bonsai computation. *)
  -> ?options:Options.t Bonsai.Value.t
  -> ?with_graph:(Graph.t -> unit)
       (** This hook may be useful if you want to, for example, bind the graph to some
           global variable on the window. That way you can poke at the graph in the
           console. *)
  -> ?on_zoom:
       (Graph.t
        -> xmin:float
