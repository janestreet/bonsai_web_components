(** An API that allows one to read files from the client's local disk.
    ...
    1. Using the companion library [Bonsai_web_file_from_web_file], which uses the Web
...
module Read_on_change : sig
  (** Functions that take bonsai values that contain [t], and kicking off reads whenever
      ...
      [Bonsai_web_form.Elements.File_picker]. So you can do things like:

      {[
        let%sub file_picker = Bonsai_web_form.Elements.File_select.single () in
        let%sub file_from_form =
          let%arr file_picker in
          Bonsai_web_form.value file_picker |> Or_error.ok
        in
        let%sub result =
          Bonsai_web_file.Read_on_change.create_single_opt file_from_form
        in
        match%sub result with
        | None -> Bonsai.const None
        | Some (filename, (Bonsai_web_file.Read_on_change.Status.Starting | In_progress _))
          ->
          let%arr filename in
