...
let create ?(mode = (`Raw_contents : [ `Raw_contents | `As_data_url ])) file =
  let read =
    Ui_effect.of_sync_fun (fun on_progress ->
        ...
        Bonsai_web.Effect.of_deferred_fun'
          (fun () ~on_exn ->
            let call_on_progress ev =
              ...
              then
                on_progress
                  { Bonsai_web_file.Progress.loaded = ev##.loaded; total = ev##.total }
            ...
            := Dom.handler (fun _ev ->
                 ...
                 Ivar.fill_if_empty
                   result
                   (Error (Bonsai_web_file.Read_error.Error error));
                 Js._true);
            file_reader##.onload
            := Dom.handler (fun ev ->
                 call_on_progress ev;
      ...
      { Bonsai_web_file.Expert.result; abort })
  in
  Bonsai_web_file.Expert.create ~read ~filename:(File.filename file |> Js.to_string)
;;
