...
(** [return] produces a bonsai form that will always produce the same value. [set] and
    [normalize] will do nothing to the form provided by this. *)
val return
  :  here:[%call_pos]
  -> ?sexp_of_t:('a -> Sexp.t)
  -> ?equal:('a -> 'a -> bool)
...
(** [project] is the powerhouse of the library; Using this function, you can change the
    ...
    [extend_view_with_error] is only called with errors that come from _this_ call to
    [project] *)
val project
...
(** [optional] takes a [('a, 'view) t] and produces a ['a option t] when given a "some
    detector" and a token "none" value. [is_some none] must be false.
