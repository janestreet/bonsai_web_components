...
module Variant : sig
  ...
  (** Creates a form that selects a set of values from a variant type, using the
      typed_variants ppx. Each variant is presented as a separate field in a form that
      select a set of values of that variant. *)
  val make_set
    :  (module S_set
          with type t = 'a
  ...
  (** Like [make_set], but don't tag variants' errors with their name. This can be useful
      when you present a label that doesn't match the variant's name. If you still want to
      tag errors on individual variants, you can do so by manually calling
      [Form.map_error] in your [form_for_variant] function. *)
