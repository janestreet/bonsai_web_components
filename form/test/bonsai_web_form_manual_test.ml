...
module%test [@name "file pickers"] _ = struct
  ...
  let mock_bonsai_file ~name : Bonsai_web_file.t =
    Bonsai_web_file.For_testing.(
  ...
  let%expect_test "single: selecting a file" =
    ...
    in
    let handle =
      Handle.create (form_result_spec [%sexp_of: Bonsai_web_file.t option]) component
    in
    Handle.show handle;
    [%expect
      {|
  ...
  let%expect_test "list: selecting multiple files" =
    ...
    let handle =
      Handle.create
        (form_result_spec [%sexp_of: Bonsai_web_file.t Filename.Map.t])
        component
    in
    Handle.show handle;
    [%expect
    ...
    let%expect_test "monomorphize variant" =
      ...
      [%expect
        ...
        ("Form.return was set, but setting is ignored."
         lib/bonsai_web/form/test/bonsai_web_form_manual_test.ml:LINE:COL)
        (Ok (Foo 5))
