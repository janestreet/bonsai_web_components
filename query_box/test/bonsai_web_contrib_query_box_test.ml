...
let get_vdom = Bonsai_web_contrib_query_box.view
let fruits = [ "apple"; "orange"; "kiwi"; "dragon fruit" ]
let items = List.mapi fruits ~f:Tuple2.create |> Int.Map.of_alist_exn

module Spec = struct
  type t = int Bonsai_web_contrib_query_box.t
  ...
  let view box =
    ...
    in
    let focused_item =
      match box.Bonsai_web_contrib_query_box.focused_item with
      | None -> "None"
...
let%expect_test "changing text does filtering" =
  let handle =
    create
      ~on_hover_item:
        (Bonsai.return Bonsai_web_contrib_query_box.On_hover_item.Focus_hovered_item)
      ()
  in
  Handle.show handle;
  [%expect
...
let%expect_test {|key stays on the same item if the list of items changes (collation)|} =
  ...
  let component =
    Bonsai_web_contrib_query_box.create
      (module Bonsai_web_contrib_query_box.Collate_map_with_score.Scored_key.M (Int))
      ~max_visible_items:(Bonsai.return 3)
      ~f:(fun query ->
        Bonsai_web_contrib_query_box.Collate_map_with_score.collate
          (module Int)
  ...
  let handle =
    let component (local_ graph) =
      Bonsai_web_contrib_query_box.create
        (module Int)
        ~expand_direction:
          (Bonsai.return Bonsai_web_contrib_query_box.Expand_direction.Down)
        ~max_visible_items:(Bonsai.return 3)
        ~f:(fun query (local_ _graph) ->
          let%arr query in
          Map.filter items ~f:(String.is_prefix ~prefix:query) |> Map.map ~f:Node.text)
...
let bisimulate_both_focus
  ~(f :
      Bonsai_web_contrib_query_box.On_focus.t
      -> expect_diff:
...
let bisimulate_hover_behaviors
  ~(f :
      Bonsai_web_contrib_query_box.On_hover_item.t
      -> expect_diff:
