...
let make_shortcut_state ~osx_behavior ~hint_trigger_keys ~parent_inject ~mode graph =
...
module Expert = struct
  ...
  let get_event_handler_for_nearest_installation graph =
...
let install_listener
  ...
  ?hint_trigger_keys
  ~mode
  (inside : Bonsai.graph -> Vdom.Node.t Bonsai.t)
  graph
...
*)
let register ?(prevent_default = Bonsai.return true) ~effect keystroke graph =
