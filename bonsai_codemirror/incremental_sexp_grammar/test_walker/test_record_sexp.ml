...
module%test [@name "Basic record"] _ = struct
  module Tester = Utils.Make_sexp_tester (struct
      type t =
        ...
        ; baz : string
        ; qux : bool [@sexp.bool]
        ; qaz : float list [@sexp.list]
        ; faa : int * bool * float list * string
        }
       ...
       (List
        (Fields
         ((allow_extra_fields false)
          (fields
           ((No_tag
             ((name foo) (required true) (args (Cons (List (Many Bool)) Empty))))
            (No_tag ((name bar) (required true) (args (Cons Integer Empty))))
            (No_tag ((name baz) (required true) (args (Cons String Empty))))
            (No_tag ((name qux) (required false) (args Empty)))
            (No_tag
             ((name qaz) (required false) (args (Cons (List (Many Float)) Empty))))
            (No_tag
             ((name faa) (required true)
              (args
               (Cons
                (List
                 (Cons Integer
                  (Cons Bool (Cons (List (Many Float)) (Cons String Empty)))))
                Empty))))))))))
      |}]
  ;;
  ...
  let%expect_test "valid record" =
    Tester.print_test_t
      "((foo (true false)) (bar 42) (baz \"hello, world!\") (qux) (faa (1 true (1.0 2.0) \
       whoah)))";
    [%expect {| |}]
  ;;
  ...
  let%expect_test "incorrect size list for record" =
    Tester.print_test_t
      {|((foo (true false))
       (bar 42)
       (baz hello world!)
       (faa (1 true (1.0 2.0) whoah)))|};
    [%expect
      {|
      Unexpected argument: this record does not expect any more arguments.
      Parsed: `world!`.
  ...
  let%expect_test "incorrect size list for boolean field of sexp.bool" =
    ...
    [%expect
      {|
      No payload for this arg expected!
  ...
  let%expect_test "misspelled record field" =
    ...
    [%expect
      ...
      Parsed: `barz`.
      Possible spelling mistake; maybe you meant `bar`, `baz` or `qaz`?
      ...
      3|        (baz hello world!))
                           ^^^^^^
      Missing required fields: bar, faa
  ...
  let%expect_test "missing record fields" =
    ...
    [%expect
      {|
      Missing required fields: bar, baz, faa
  ...
  let%expect_test "extra record field" =
    Tester.print_test_t
      {|((foo (true false)) (bar 42) (baz hello) (extra field)
       (faa (1 true (1.0 2.0) whoah)))|};
    [%expect
      {|
      Invalid field name, and additional fields are not allowed.
  ...
  let%expect_test "duplicate record field" =
    Tester.print_test_t
      {|((foo (true false)) (bar 42) (bar 42) (baz hello)
       (faa (1 true (1.0 2.0) whoah)))|};
    [%expect
      {|
  ...
  let%expect_test "propagates errors" =
    Tester.print_test_t
      "((foo (true false)) (bar 42.0) (baz \"hello, world!\")\n\
      \       (faa (1 true (1.0 2.0) whoah)))";
    [%expect
      {|
