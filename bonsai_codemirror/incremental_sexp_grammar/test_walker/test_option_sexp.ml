...
module%test [@name "lesser-known option representations"] _ = struct
  ...
  let%expect_test "all option formats are accepted" =
    List.iter
      [ "()"; "(test)"; "None"; "(Some test)"; "(Some)"; "none"; "(some test)"; "(some)" ]
      ~f:Tester.print_test_t;
    [%expect {| |}]
  ;;
end

module%test [@name "basic option"] _ = struct
  module Tester = Utils.Make_sexp_tester (struct
...
module%test [@name "option of record"] _ = struct
  module X = struct
    type t =
      { a : bool
      ; b : int
