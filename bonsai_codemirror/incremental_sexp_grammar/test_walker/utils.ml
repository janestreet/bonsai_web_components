...
let quick_test_noncompliant_from_gen ?trials grammar =
  let generator =
    Sexp_grammar_validation.Private.Disobedient_generator.Private.create_with_max_tries
      ~max_tries:1_000
      grammar
  in
  Quickcheck.test ?trials generator ~f:(function
    | Error `Max_tries_exceeded -> (* Grammar accepts all sexps! *) ()
    | Ok (x, _) -> assert_errors (Sexp.to_string x) ~grammar:grammar.untyped)
;;

let quickcheck_tests_no_type ?trials grammar =
  quick_test_compliant_from_gen ?trials grammar;
  if not (Sexp_grammar.known_to_accept_all_sexps grammar)
  then quick_test_noncompliant_from_gen ?trials grammar
;;
...
module Make_sexp_tester (M : S) = struct
  type t = M.t [@@deriving sexp, sexp_grammar, quickcheck]
  ...
  let print_grammar_sexp () =
    print_s [%sexp (M.t_sexp_grammar.untyped : Sexp_grammar.grammar)]
  ;;
end
