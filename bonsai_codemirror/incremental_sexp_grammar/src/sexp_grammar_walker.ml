...
type intermediate_t =
  ...
  *)
  ...
  | Consume_args of
      { args : Sexp_grammar.list_grammar
      ; for_ : [ `Variant | `Record ]
      }
  ...
  (* Consumes a single ). *)
  | Consume_rparen
  (* Pops the topmost tycon context frame from the stack. *)
  | Remove_context
[@@deriving sexp_of]
...
and t =
  ...
  ; tycons_context : Tycons_context.t
...
let create grammar =
  ...
  ; tycons_context = Tycons_context.empty
...
let is_done t =
  not
    (List.exists t.stack ~f:(function
      | Remove_context | Inside_list Empty | Inside_list (Many _) | In_error _ -> false
      | _ -> true))
;;
...
let end_of_input_errors t =
  let rec loop stack acc =
    match stack with
    | [] -> acc
    | Remove_context :: rest -> loop rest acc
...
module Validate = struct
  ...
  let record_field
    ...
    | Some { args; _ }, _, `First_occurence ->
      Ok [ Consume_args { args; for_ = `Record } ]
  ;;
...
let rec process_token t_without_registering_token (token : Token.t) =
  ...
  | String s, Consume_variant_constructor variant :: rest ->
    (match Validate.variant_inside_parens variant s with
     | Ok args -> ok (Consume_args { args; for_ = `Variant } :: Consume_rparen :: rest)
     | Error err ->
       err_multi
         ~emit_error:`Now
         ~error_ends:Before_end_of_list
         ~stack:(Consume_rparen :: rest)
         err)
  | (String _ | Lparen), Consume_args { args; for_ } :: rest ->
    (match args with
     | Cons (next, Empty) ->
       let for_ =
         match for_ with
       ...
       consume
         (Full_grammar next
          :: Consume_no_more_args
               (Lint_error.of_tokens_lazy_err
                  (lazy
                    [%string
                      "Unexpected argument: this %{for_} does not expect any more \
                       arguments."]))
          :: rest)
     | Empty ->
       curr_tok_err
         ~stack:rest
         (Lint_error.at_pos_of_first_token [%string "No payload for this arg expected!"])
     | _ -> consume (Inside_list args :: rest))
  | Rparen, Consume_args { args; for_ } :: rest ->
    let all_fields_optional =
      List.for_all ~f:(fun field ->
        let { Sexp_grammar.required; _ } = strip_tags field in
        not required)
    in
    (match args with
     | Empty | Many _ -> consume rest
     | Fields { fields; _ } when all_fields_optional fields -> consume rest
     | _ ->
       let rest =
         match rest with
         | Consume_rparen :: rest -> rest
         | _ -> rest
       in
       let inner = Grammar_tools.arg_to_string args in
       let for_ =
       ...
       curr_tok_err
         ~stack:rest
         (Lint_error.at_pos_of_first_token
            [%string "This %{for_} requires a payload of type %{inner}."]))
  (* Recursive types *)
  | _, Full_grammar (Tycon (name, args, defns)) :: rest ->
    let (key : Tycons_context.Tycon.t) = { name; args; defns } in
    (match Tycons_context.register t.tycons_context key with
     | Error error ->
       err_multi
         ~emit_error:`Now
         ~stack:rest
         (Lint_error.at_pos_of_first_token (Error.to_string_hum error))
     | Ok (chosen_defn_grammar, new_tycons_context) ->
       process_token
         { t with
           stack = Full_grammar chosen_defn_grammar :: Remove_context :: rest
         ; tycons_context = new_tycons_context
         }
         token)
  | _, Remove_context :: rest ->
    (match Tycons_context.pop t.tycons_context with
     | Ok new_tycons_context ->
       process_token { t with stack = rest; tycons_context = new_tycons_context } token
     | Error error ->
       err_multi
         ~stack:rest
         ~emit_error:`Now
  ...
  | _, Full_grammar (Tyvar id) :: rest ->
    (match Tycons_context.resolve t.tycons_context id with
     | Ok grammar -> consume (Full_grammar grammar :: rest)
     | Error err ->
       err_multi
         ~stack:rest
         ~emit_error:`Now
         (Lint_error.at_pos_of_first_token (Error.to_string_hum err)))
  | _, Full_grammar (Recursive (name, args)) :: rest ->
    (match Tycons_context.curr_defns t.tycons_context with
     | Some defns -> consume (Full_grammar (Tycon (name, args, defns)) :: rest)
     | None ->
       curr_tok_err
         ~stack:rest
         (Lint_error.of_tokens
...
module For_testing = struct
  ...
  let assert_nothing_too_crazy { stack; tycons_context; _ } =
    let stack_size = List.length stack in
    if stack_size > 1_000
    then print_s [%message "Suspiciously large stack" (stack_size : int)];
    let num_tycon_frames = Tycons_context.For_testing.num_frames tycons_context in
    if num_tycon_frames > 50
    then
      print_s
        [%message
          "Suspiciously large number of tycon frames; are we recursing more than we need \
           to? "
            (num_tycon_frames : int)]
  ;;
end
