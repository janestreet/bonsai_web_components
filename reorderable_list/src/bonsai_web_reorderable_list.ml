...
let list
  ...
  ~(dnd : (src, int) Bonsai_web_drag_and_drop.t Bonsai.t)
  ?(enable_debug_overlay = false)
  ?(extra_item_attrs = Bonsai.return Attr.empty)
  ?(container_extra_attrs = Bonsai.return Attr.empty)
  ...
  let%sub sizes, size_attr =
    Bonsai_web_element_size_hooks.Bulk_size_tracker.component
      (module Key)
      Prune_stale
      graph
  in
  ...
  and is_dragging
  and total_height
  and empty_list_placeholder
  and container_extra_attrs in
...
let with_inject
  ...
  in
  let dnd =
    Bonsai_web_drag_and_drop.create
      ...
      graph
  in
  let source = dnd >>| Bonsai_web_drag_and_drop.source in
